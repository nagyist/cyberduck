...
public class CryptoFTPSingleTransferWorkerTest extends AbstractFTPTest {
    ...
    public void testUpload() throws Exception {
        ...
        assertEquals(content.length, cryptomator.getFeature(session, AttributesFinder.class, new DefaultAttributesFinderFeature(session)).find(file1).getSize());
        {
            ...
            final InputStream in = new CryptoReadFeature(session, new FTPReadFeature(session), cryptomator).read(file1, new TransferStatus().setLength(content.length), ConnectionCallback.noop);
