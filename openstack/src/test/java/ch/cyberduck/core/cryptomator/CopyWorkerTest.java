...
public class CopyWorkerTest extends AbstractSwiftTest {
    ...
    public void testCopyToDifferentFolderCryptomator() throws Exception {
        ...
        new CryptoTouchFeature<>(session, new DefaultTouchFeature<StorageObject>(
                session), cryptomator).touch(new CryptoWriteFeature<>(session, new SwiftWriteFeature(session, new SwiftRegionService(session)), cryptomator), source, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(source));
    ...
    public void testCopyFolder() throws Exception {
        ...
        session.withRegistry(new DefaultVaultRegistry(new DisabledPasswordCallback(), cryptomator));
        cryptomator.getFeature(session, Directory.class, new SwiftDirectoryFeature(session)).mkdir(
                cryptomator.getFeature(session, Write.class, new SwiftWriteFeature(session, new SwiftRegionService(session))), folder, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(folder));
        new CryptoTouchFeature<>(session, new DefaultTouchFeature<StorageObject>(
                session), cryptomator).touch(new CryptoWriteFeature<>(session, new SwiftWriteFeature(session, new SwiftRegionService(session)), cryptomator), file, new TransferStatus());
    ...
    public void testCopyFileOutsideVault() throws Exception {
        ...
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(encryptedFolder));
        new CryptoTouchFeature<>(session, new DefaultTouchFeature<StorageObject>(
                session), cryptomator).touch(new CryptoWriteFeature<>(session, new SwiftWriteFeature(session, new SwiftRegionService(session)), cryptomator), encryptedFile, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(encryptedFile));
