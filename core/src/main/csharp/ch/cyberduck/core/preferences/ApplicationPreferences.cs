...
namespace Ch.Cyberduck.Core.Preferences
{
    public class ApplicationPreferences : CorePreferences
    {
        ...
        public ApplicationPreferences(Locales locales)
        {
            this.locales = locales;

            userConfig = new(Path.Combine(LocalSupportDirectoryFinder.Directory, $"{RuntimeInfo.ProductName}.user.config"));
        }
        ...
        protected override void setDefaults()
        {
            ...
            this.setDefault("application.name", RuntimeInfo.ProductName);
            this.setDefault("application.datafolder.name", RuntimeInfo.DataFolderName);
            this.setDefault("oauth.handler.scheme",
                String.Format("x-{0}-action", StringUtils.deleteWhitespace(RuntimeInfo.ProductName.ToLower())));

            this.setDefault("application.version", RuntimeInfo.VersionString);
            this.setDefault("application.revision", RuntimeInfo.Revision);
