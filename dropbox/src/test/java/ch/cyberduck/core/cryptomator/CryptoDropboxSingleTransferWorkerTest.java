...
public class CryptoDropboxSingleTransferWorkerTest extends AbstractDropboxTest {
    ...
    public void testUpload() throws Exception {
        ...
        assertTrue(cryptomator.getFeature(session, Find.class, new DropboxFindFeature(session)).find(dir1));
        assertEquals(content.length, cryptomator.getFeature(session, AttributesFinder.class, new DefaultAttributesFinderFeature(session)).find(file1).getSize());
        {
            ...
            final InputStream in = new CryptoReadFeature(session, new DropboxReadFeature(session), cryptomator).read(file1, new TransferStatus().setLength(content.length), ConnectionCallback.noop);
        ...
        cryptomator.getFeature(session, Delete.class, new DropboxDeleteFeature(session)).delete(Arrays.asList(file1, file2, dir1, vault), LoginCallback.noop, new Delete.DisabledCallback());
        localFile1.delete();
        localFile2.delete();
