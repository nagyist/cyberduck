...
public class CopyWorkerTest extends AbstractDriveTest {
    ...
    public void testCopyFile() throws Exception {
        ...
        worker.run(session);
        assertTrue(cryptomator.getFeature(session, Find.class, new DriveFindFeature(session, fileid)).find(source));
        assertTrue(cryptomator.getFeature(session, Find.class, new DriveFindFeature(session, fileid)).find(target));
    ...
    public void testCopyToDifferentFolderCryptomator() throws Exception {
        ...
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(source));
        cryptomator.getFeature(session, Directory.class, new DriveDirectoryFeature(session, fileid)).mkdir(
                cryptomator.getFeature(session, Write.class, new DriveWriteFeature(session, fileid)), targetFolder, new TransferStatus());
    ...
    public void testCopyFolder() throws Exception {
        ...
        final DriveFileIdProvider fileid = new DriveFileIdProvider(session);
        cryptomator.getFeature(session, Directory.class, new DriveDirectoryFeature(session, fileid)).mkdir(
                cryptomator.getFeature(session, Write.class, new DriveWriteFeature(session, fileid)), folder, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(folder));
        new CryptoTouchFeature<>(session, new DefaultTouchFeature<File>(session), cryptomator).touch(
                cryptomator.getFeature(session, Write.class, new DriveWriteFeature(session, fileid)), file, new TransferStatus());
        ...
        assertTrue(cryptomator.getFeature(session, Find.class, new DriveFindFeature(session, fileid)).find(file));
        assertTrue(cryptomator.getFeature(session, Find.class, new DriveFindFeature(session, fileid)).find(fileRenamed));
        // copy folder
        ...
        assertTrue(cryptomator.getFeature(session, Find.class, new DriveFindFeature(session, fileid)).find(folder));
        assertTrue(cryptomator.getFeature(session, Find.class, new DriveFindFeature(session, fileid)).find(folderRenamed));
    ...
    public void testCopyFileIntoVault() throws Exception {
        ...
        final DriveFileIdProvider fileid = new DriveFileIdProvider(session);
        new DriveTouchFeature(session, fileid).touch(new DriveWriteFeature(session, fileid), cleartextFile, new TransferStatus());
        ...
        session.withRegistry(new DefaultVaultRegistry(new DisabledPasswordCallback(), cryptomator));
        cryptomator.getFeature(session, Directory.class, new DriveDirectoryFeature(session, fileid)).mkdir(
                cryptomator.getFeature(session, Write.class, new DriveWriteFeature(session, fileid)), encryptedFolder, new TransferStatus());
        ...
        worker.run(session);
        assertTrue(new DriveFindFeature(session, fileid).find(cleartextFile));
        assertTrue(cryptomator.getFeature(session, Find.class, new DriveFindFeature(session, fileid)).find(encryptedFile));
        session.getRegistry().clear();
        new DeleteWorker(LoginCallback.noop, Collections.singletonList(vault), ProgressListener.noop).run(session);
    }
    ...
    public void testCopyDirectoryIntoVault() throws Exception {
        ...
        final DriveFileIdProvider fileid = new DriveFileIdProvider(session);
        new DriveDirectoryFeature(session, fileid).mkdir(new DriveWriteFeature(session, fileid), cleartextFolder, new TransferStatus());
        new DriveTouchFeature(session, fileid).touch(new DriveWriteFeature(session, fileid), cleartextFile, new TransferStatus());
    ...
    public void testCopyFileOutsideVault() throws Exception {
        ...
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(encryptedFolder));
        new CryptoTouchFeature<>(session, new DefaultTouchFeature<File>(session), cryptomator).touch(
                cryptomator.getFeature(session, Write.class, new DriveWriteFeature(session, fileid)), encryptedFile, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(encryptedFile));
    ...
    public void testCopyDirectoryOutsideVault() throws Exception {
        ...
        worker.run(session);
        assertTrue(cryptomator.getFeature(session, Find.class, new DriveFindFeature(session, fileid)).find(encryptedFolder));
        assertTrue(cryptomator.getFeature(session, Find.class, new DriveFindFeature(session, fileid)).find(encryptedFile));
        assertTrue(new DriveFindFeature(session, fileid).find(cleartextFolder));
